...
7. Use `$orchestrate-workers`, `$manage-worker-protocol`, `$process-orchestration-inbox`, `$integrate-worker-output`, `$audit-worker-output`, `$guide-orchestration`, and `$recover-orchestration` when their triggers match.
8. Treat worker output as evidence; capture, validate, integrate/reject, then archive only after consumption.
9. End each loop with updated `state.md`, `workers.md`, and the next single action.
