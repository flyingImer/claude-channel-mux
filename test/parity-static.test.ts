...
test('plugin ships CCM orchestration role skills', () => {
  ...
  expect(readFileSync('skills/orchestrate-workers/SKILL.md', 'utf8')).toContain('orchestration meta-work')
  expect(readFileSync('skills/orchestrate-workers/SKILL.md', 'utf8')).toContain('Worker Rooms may use dynamic workflow or internal fan-out as a worker-local quality/throughput technique')
  expect(readFileSync('skills/orchestrate-workers/SKILL.md', 'utf8')).toContain('Workers must not inherit room creation/adoption/archive authority')
  expect(readFileSync('skills/work-in-worker-room/SKILL.md', 'utf8')).toContain('Worker Report')
  expect(readFileSync('skills/work-in-worker-room/SKILL.md', 'utf8')).toContain('If the task arrived through a runtime wrapper')
  expect(readFileSync('skills/work-in-worker-room/SKILL.md', 'utf8')).toContain('Treat assigned synthesis work as the primary Worker Task')
  ...
  expect(readFileSync('skills/manage-worker-protocol/SKILL.md', 'utf8')).toContain('Claude worker goal setup must be sent with `send_worker_raw_command` as `/goal create dynamic workflow to <task specific goal description> /think-harder /superpowers:verification-before-completion`')
  expect(readFileSync('skills/manage-worker-protocol/SKILL.md', 'utf8')).toContain('Codex worker goal setup must be sent with `send_worker_raw_command` as `/goal $superpowers:subagent-driven-development <task specific goal description> $think-harder $superpowers:verification-before-completion`')
  expect(readFileSync('skills/manage-worker-protocol/SKILL.md', 'utf8')).toContain('Do not include either `/goal` setup command in `send_worker_task`')
  expect(readFileSync('skills/manage-worker-protocol/SKILL.md', 'utf8')).toContain('Synthesis-related work always requires a dedicated Worker Room')
  expect(readFileSync('skills/manage-worker-protocol/SKILL.md', 'utf8')).toContain('Internal Throughput')
  expect(readFileSync('skills/manage-worker-protocol/SKILL.md', 'utf8')).toContain('internal fan-out is worker-local only')
  ...
  expect(readFileSync('skills/bootstrap-git-orchestration/SKILL.md', 'utf8')).toContain('Stop after bootstrap/adopt/repair validation')
  expect(readFileSync('skills/bootstrap-git-orchestration/SKILL.md', 'utf8')).toContain('Do not dispatch workers')
  expect(readFileSync('skills/bootstrap-git-orchestration/SKILL.md', 'utf8')).toContain('Codex native subagents')
  ...
  expect(readFileSync('skills/import-gp-packet/SKILL.md', 'utf8')).toContain('Stop after raw packet persistence')
  expect(readFileSync('skills/import-gp-packet/SKILL.md', 'utf8')).toContain('Do not dispatch workers')
...
test('daemon agent-facing tool errors use shared redaction helper', () => {
  const daemon = readFileSync('daemon.ts', 'utf8')
  expect(daemon).toContain("sendToIpcConn(route.responseUuid, callerConn, { type: 'tool_error', callId: msg.callId, error: errorMessage(err) }, 'tool error')")
...
test('zellij session detection uses exact session names', () => {
  const daemon = readFileSync('daemon.ts', 'utf8')
  expect(daemon).toContain("findZellijSessionLine, zellijAttachCommand } from './zellij.js'")
  ...
  expect(daemon).toContain('async function ensureDetachedZellijSession(sessionName: string, label: string): Promise<void>')
  expect(daemon).toContain('const ZELLIJ_BOOTSTRAP_ROWS = 60')
  expect(daemon).toContain('const ZELLIJ_BOOTSTRAP_COLS = 100')
  expect(daemon).toContain('stty rows ${ZELLIJ_BOOTSTRAP_ROWS} cols ${ZELLIJ_BOOTSTRAP_COLS}')
  expect(daemon).toContain("args[0] === 'list-sessions' && errorMessage(err).includes('No active zellij sessions found.')")
  expect(daemon).toContain('stopDetachedZellijBootstrapClient(backendSessionName)')
  expect(daemon).toContain('stopDetachedZellijBootstrapClient(sessionName)')
  expect(daemon).toContain("const CLAUDE_EFFORT = (process.env.CHANNEL_DAEMON_CLAUDE_EFFORT ?? 'ultracode').trim()")
  expect(daemon).toContain("const effortArgs = CLAUDE_EFFORT ? ['--effort', CLAUDE_EFFORT] : []")
  expect(daemon).toContain('findPaneByTabNameInSession(backendSessionName, tabName)')
...
test('daemon zellij and worktree helpers avoid shell interpolation', () => {
  ...
  expect(daemon).toContain("await zellijActionInSessionAsync(backendSessionName, ['new-tab', '--name', tabName, '--', 'bash', '-c'")
...
test('escort zellij helpers avoid shell interpolation for pane inputs', () => {
  ...
  expect(escort).toContain("execFileSync('zellij', ['--session', sessionName, 'action', ...args]")
  expect(escort).toContain("execFileAsync('zellij', ['--session', sessionName, 'action', 'dump-screen', '--pane-id', String(paneId)]")
  expect(escort).toContain("zjForSession(sessionName, 'write-chars', '--pane-id', String(paneId), text)")
...
test('escort per-session zellij pane lookup does not require default ccmux to be alive', () => {
  const escort = readFileSync('escort.ts', 'utf8')
  const listPanesBlock = escort.slice(escort.indexOf('export function listPanesInSession'), escort.indexOf('export function findPaneByTabName'))
  expect(listPanesBlock).toContain('zellijTargetSessionAlive(sessionName)')
  expect(listPanesBlock).not.toContain('zellijSessionAlive()')
})
...
test('escort dump-screen failures are observable while preserving empty fallback', () => {
  ...
  expect(block).toContain('escort: dump-screen failed for ${sessionName} pane ${paneId}: ${errorMessage(err)}')
  expect(block).toContain('escort: async dump-screen failed for ${sessionName} pane ${paneId}: ${errorMessage(err)}')
...
test('escort zellij write helpers log failures instead of silently dropping UX actions', () => {
  ...
  expect(helperBlock).toContain('escort: send-keys failed for ${sessionName} pane ${paneId}: ${errorMessage(err)}')
  expect(helperBlock).toContain('escort: write raw failed for ${sessionName} pane ${paneId}: ${errorMessage(err)}')
  expect(helperBlock).toContain('escort: write chars failed for ${sessionName} pane ${paneId}: ${errorMessage(err)}')
  expect(helperBlock).toContain('escort: close tab ${JSON.stringify(tabName)} in ${sessionName} failed: ${errorMessage(err)}')
...
test('daemon Claude nav callbacks validate select indexes and keys', () => {
  ...
  expect(daemon).toContain('const targetPane = codexPane ?? claudePane')
  expect(daemon).toContain("sendKeysInSession(targetPane.sessionName, targetPane.paneId, parsed.action.key)")
  expect(daemon).toContain('parsed.action.type ===')
  expect(daemon).toContain('async function navigateAndConfirm(paneId: number, targetIdx: number, sessionName = ZELLIJ_SESSION): Promise<boolean>')
...
test('Agent Control Path worker tasks do not use synthetic ids as platform thread anchors', () => {
  const daemon = readFileSync('daemon.ts', 'utf8')
  expect(daemon).toContain("function platformMessageAnchor(ck: string, id?: string): string | undefined")
  expect(daemon).toContain("if (!id || id.startsWith('acp:')) return undefined")
  expect(daemon).toContain("if (platform === 'slack' && !/^\\d+\\.\\d+$/.test(id)) return undefined")
  const deliverBlock = daemon.slice(daemon.indexOf('async function deliverUserTurn'), daemon.indexOf('async function onMessage'))
  expect(deliverBlock).toContain('const platformMessageId = platformMessageAnchor(ck, msg.messageId)')
  expect(deliverBlock).toContain('const platformThreadId = platformMessageAnchor(ck, msg.replyToId ?? msg.messageId)')
  expect(deliverBlock).toContain('const turnNoticeOpts = platformThreadId ? { replyTo: platformThreadId, broadcast: true } : undefined')
  expect(deliverBlock).toContain('if (platformMessageId) {')
  expect(deliverBlock).toContain("adapter?.addReaction(id, platformMessageId, '👀').catch(err =>")
  expect(deliverBlock).toContain('if (platformThreadId) {')
  expect(deliverBlock).toContain('const threadId = platformThreadId ??')
  expect(deliverBlock).toContain('...(platformThreadId ? { reply_to_id: platformThreadId } : {})')
})
...
test('daemon pid file write failure logs and cleans up IPC socket', () => {
  ...
  expect(block).toContain("unlinkOwnedDaemonPath(SOCK_PATH, 'unlink IPC socket after pid file failure')")
  expect(block).toContain("unlinkOwnedDaemonPath(LOCK_FILE, 'unlink daemon lock after pid file failure')")
  expect(block).toContain('process.exit(1)')
})

test('daemon startup refuses duplicate live pid and recovers stale locks', () => {
  const daemon = readFileSync('daemon.ts', 'utf8')
  const block = daemon.slice(daemon.indexOf('const existingDaemonPid = numericPidFromFile(PID_FILE)'), daemon.indexOf('// ---------------------------------------------------------------------------\n// Transcript polling'))
  expect(block).toContain('isProcessAlive(existingDaemonPid)')
  expect(block).toContain('refusing to start duplicate')
  expect(block).toContain("openSync(LOCK_FILE, 'wx', 0o600)")
  expect(block).toContain('isProcessAlive(lockOwner)')
  expect(block).toContain("unlinkSync(LOCK_FILE)")
  expect(daemon.indexOf('const existingDaemonPid = numericPidFromFile(PID_FILE)')).toBeLessThan(daemon.indexOf('cleanExitedTabs()'))
})
...
test('shared Codex bridge accepts concurrent app-server bridges without disconnecting callers', () => {
  const daemon = readFileSync('daemon.ts', 'utf8')
  const block = daemon.slice(daemon.indexOf('if (l.ipcConn && l.ipcConn !== conn && !l.ipcConn.destroyed)'), daemon.indexOf('const firstEver = !announcedReconnect.has(uuid)'))
  expect(block).toContain('if (uuid === SHARED_CODEX_BRIDGE_ID)')
  expect(block).toContain('accepted additional shared Codex bridge register')
  expect(block).toContain("sendToIpcConn(uuid, conn, { type: 'registered', uuid, channels: [] }, 'shared bridge secondary registered')")
  expect(block).not.toContain('destroyIpcConn(l.ipcConn, `replace shared Codex bridge ${uuid.slice(0, 8)}`)')
  expect(block).toContain('} else {')
})

test('shared Codex bridge tool replies are returned on the caller socket', () => {
  const daemon = readFileSync('daemon.ts', 'utf8')
  const handleTool = daemon.slice(daemon.indexOf('async function handleTool'), daemon.indexOf('// ---------------------------------------------------------------------------\n// Permission request'))
  expect(handleTool).toContain('callerConn: Socket')
  expect(handleTool).toContain("sendToIpcConn(callerUuid, callerConn, { type: 'tool_error'")
  expect(handleTool).toContain("sendToIpcConn(route.responseUuid, callerConn, { type: 'tool_result'")
  expect(handleTool).toContain("sendToIpcConn(route.responseUuid, callerConn, { type: 'tool_error'")
  expect(handleTool).not.toContain("sendToLive(route.responseUuid, { type: 'tool_result'")
})
...
test('tool calls reject stale cross-room chat_id values', () => {
  ...
  const handleTool = daemon.slice(daemon.indexOf('async function handleTool'), daemon.indexOf('// ---------------------------------------------------------------------------\n// Permission request'))
  expect(handleTool).toContain('route = resolveToolCallRoute(callerUuid, msg.tool, msg.args)')
  expect(handleTool).toContain("sendToIpcConn(callerUuid, callerConn, { type: 'tool_error', callId: msg.callId, error: errorMessage(err) }, 'tool route error')")
  ...
  expect(daemon).toContain('orchestratorToolChannelKey(callerUuid, requestedCk)')
  expect(handleTool.indexOf('route = resolveToolCallRoute(callerUuid, msg.tool, msg.args)')).toBeLessThan(handleTool.indexOf('const adapter = adapterFor(ck)'))
  expect(handleTool).not.toContain('const ck = stringValue(msg.args.chat_id)')
})

test('shared Codex app-server tool calls are routed by bound room session', () => {
  const daemon = readFileSync('daemon.ts', 'utf8')
  const driver = readFileSync('agents/codex/app-server-driver.ts', 'utf8')
  const server = readFileSync('server.ts', 'utf8')
  const resolver = daemon.slice(daemon.indexOf('function resolveToolCallRoute'), daemon.indexOf('function roomCwd'))
  expect(driver).toContain("const SHARED_CODEX_BRIDGE_ID = 'ccm-shared-codex-app-server'")
  ...
  expect(daemon).not.toContain('CODEX_ROOM_TOKENS_FILE')
  expect(daemon).not.toContain('newCodexRoomToken')
  expect(daemon).not.toContain('invalidateCodexRoomToken')
  expect(resolver).toContain('callerUuid === SHARED_CODEX_BRIDGE_ID')
  expect(resolver).toContain("const requestedCk = stringValue(args.chat_id)")
  expect(resolver).toContain("const sessionId = bindingUuid(requestedCk, 'codex')")
  expect(resolver).toContain("reason: 'shared_codex_room_not_bound'")
...
test('Codex goal command keeps CCM room metadata in the turn envelope', () => {
  const driver = readFileSync('agents/codex/app-server-driver.ts', 'utf8')
...
test('path binding and slash passthrough confirmations use observable channel notice helper', () => {
  ...
  expect(daemon).toContain('create_room_with_bot_invited|bind_worker_room|start_worker_agent|send_worker_raw_command|send_worker_task|capture_worker_report')
  ...
  expect(slashBlock).toContain('await sendClaudePaneRawCommand(ck, uuid, commandText)')
  const rawHelper = daemon.slice(daemon.indexOf('async function sendClaudePaneRawCommand'), daemon.indexOf('async function sendCodexRawCommand'))
  expect(rawHelper).toContain('const pane = resolveClaudePaneHandle(sessionId)')
  expect(rawHelper).toContain('const writeOk = writeCharsInSession(pane.sessionName, pane.paneId, rawCommand)')
  expect(rawHelper).toContain("const enterOk = writeOk ? sendKeysInSession(pane.sessionName, pane.paneId, 'Enter') : false")
  expect(rawHelper).toContain('if (!writeOk || !enterOk)')
...
test('daemon filesystem cleanup logs unexpected errors while ignoring missing files', () => {
  ...
  expect(shutdown).toContain("unlinkOwnedDaemonPath(SOCK_PATH, 'unlink IPC socket during shutdown')")
  expect(shutdown).toContain("unlinkOwnedDaemonPath(PID_FILE, 'unlink pid file during shutdown')")
  expect(shutdown).toContain("unlinkOwnedDaemonPath(LOCK_FILE, 'unlink daemon lock during shutdown')")
...
test('tool errors clear active typing before returning to agent', () => {
  ...
  expect(block.indexOf('await clearAgentTyping(uuid)', block.indexOf('} catch (err) {'))).toBeLessThan(block.indexOf("sendToIpcConn(route.responseUuid, callerConn, { type: 'tool_error'"))
})
...
test('agent typing indicators are started and cleared on terminal paths', () => {
  ...
  expect(daemon).toContain('const platformThreadId = platformMessageAnchor(ck, msg.replyToId ?? msg.messageId)')
  expect(daemon).toContain("adapter?.addReaction(id, platformMessageId, '👀').catch(err =>")
  expect(daemon).toContain('daemon: start-turn reaction failed')
  expect(daemon).toContain('adapter?.showTyping?.(id, platformThreadId).catch(err =>')
  expect(daemon).toContain('daemon: start-turn typing failed')
  expect(daemon).toContain('const turnNoticeOpts = platformThreadId ? { replyTo: platformThreadId, broadcast: true } : undefined')
  expect(daemon).toContain('activeTypingAnchors.set(uuid, { channelKey: ck, threadId: platformThreadId })')
...
test('daemon zellij cleanup and teardown failures are observable', () => {
  ...
  expect(daemon).toContain('close zellij session failed for ${uuid.slice(0, 8)}: ${errorMessage(err)}')
...
test('daemon runtime teardown clears per-session UI state', () => {
  ...
  expect(daemon).toContain('activeTypingAnchors.delete(uuid)')
  expect(daemon).toContain('async function killSession(uuid: string, options: { runtimeHint?: AgentRuntimeKind } = {}): Promise<void>')
  expect(daemon).toContain('const l = live.get(uuid)')
  expect(daemon).toContain('if (l?.child)')
  expect(daemon).toContain('child SIGTERM failed for ${uuid.slice(0, 8)}: ${errorMessage(err)}')
  expect(daemon).toContain('} else if (zellijAvailable)')
  expect(daemon).toContain('IPC destroy failed for ${uuid.slice(0, 8)}: ${errorMessage(err)}')
...
test('/cc and /cx tui commands are intercepted by CCM before native passthrough', () => {
  const daemon = readFileSync('daemon.ts', 'utf8')
  const parseCmd = daemon.slice(daemon.indexOf('function parseCmd'), daemon.indexOf('// ---------------------------------------------------------------------------\n// Session picker'))
  const ccSubBlock = parseCmd.slice(parseCmd.indexOf("agentCommandBodyAfterPrefix(c, 'cc')"), parseCmd.indexOf('// /cx xxx'))
  const cxSubBlock = parseCmd.slice(parseCmd.indexOf("agentCommandBodyAfterPrefix(c, 'cx')"), parseCmd.indexOf('const agentsCueMatch'))

  expect(ccSubBlock).toContain("if (tuiSubM) return { t: 'tui', runtime: 'claude'")
  expect(cxSubBlock).toContain("if (tuiSubM) return { t: 'tui', runtime: 'codex'")
  expect(ccSubBlock.indexOf('const tuiSubM')).toBeLessThan(ccSubBlock.indexOf("return { t: 'slash'"))
  expect(cxSubBlock.indexOf('const tuiSubM')).toBeLessThan(cxSubBlock.indexOf("return { t: 'agent_command'"))
})
...
test('Codex explicit stop clears pending requests while stale panels remain restart-only', () => {
  ...
  const killSessionBody = daemon.slice(daemon.indexOf('async function killSession'), daemon.indexOf('function unbind('))
...
test('Codex app-server uses websocket runtime and auto-attaches remote TUI', () => {
  ...
  const lifecycle = readFileSync('agents/codex/session.ts', 'utf8')
  const bindings = readFileSync('bindings.ts', 'utf8')
  expect(config).toContain('CCM_CODEX_CONFIG_ARGS')
  expect(driver).toContain('...this.baseConfig.configArgs')
  ...
  expect(daemon).toContain("ensureSession: sessionId => ensureDetachedZellijSession(codexTuiZellijSessionName(sessionId), 'Codex TUI')")
  expect(daemon).not.toContain('ensureSession: () => ensureZellijSession()')
  ...
  expect(lifecycle).toContain('this.tui.closeTab(sessionId, this.tabName(sessionId))')
...
test('archive_room cleans up archived room runtime state after provider archive', () => {
  const daemon = readFileSync('daemon.ts', 'utf8')
  ...
  expect(archiveBlock).toContain('const resultFacts = await adapter.archiveRoom({ roomId })')
...
test('room cleanup kills persisted zellij sessions even after daemon restart', () => {
  const daemon = readFileSync('daemon.ts', 'utf8')
  const deleteRoomBlock = daemon.slice(daemon.indexOf('async function deleteRoomState'), daemon.indexOf('async function resetRoomForPathChange'))
  expect(deleteRoomBlock).toContain('await killSession(entry.uuid, { runtimeHint: entry.runtime })')
  const killBlock = daemon.slice(daemon.indexOf('async function killSession'), daemon.indexOf('function unbind('))
  expect(killBlock).toContain('runtimeHint?: AgentRuntimeKind')
  expect(killBlock).toContain('const runtime = options.runtimeHint ?? l?.runtime ?? runtimeForUuid(uuid)')
  expect(killBlock).toContain("if (runtime === 'codex')")
  expect(killBlock).toContain('codexTuiZellijSessionName(uuid)')
  expect(killBlock).toContain('claudeBackendZellijSessionName(uuid)')
  expect(killBlock).not.toContain('} else if (l && zellijAvailable)')
})
