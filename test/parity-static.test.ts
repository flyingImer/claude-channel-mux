...
test('shared Codex app-server tool calls are routed by room capability token', () => {
  ...
  expect(resolver).toContain('rawToken === SHARED_CODEX_BRIDGE_ID')
  expect(resolver).toContain("event: 'capability_token_bridge_id_used'")
  expect(resolver).toContain('ccm-shared-codex-app-server is a shared bridge id, not a ccm_room_token')
  ...
  expect(server).toContain('Never use CC_CHANNEL_SESSION_UUID')
  expect(server).toContain('ccm-shared-codex-app-server as ccm_room_token')
...
test('Codex goal command keeps CCM room metadata in the turn envelope', () => {
  ...
  expect(commandHasCcmRoomMetadata).toContain("'ccm_room_token'")
