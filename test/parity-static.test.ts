...
test('plugin ships CCM orchestration role skills', () => {
  ...
  expect(readFileSync('skills/manage-worker-protocol/SKILL.md', 'utf8')).toContain('Claude worker task text must be wrapped with `/goal create dynamic workflow to <task specific goal description> /think-harder /superpowers:verification-before-completion`')
  expect(readFileSync('skills/manage-worker-protocol/SKILL.md', 'utf8')).toContain('Codex worker task text must be wrapped with `/goal $superpowers:subagent-driven-development <task specific goal description> $think-harder $superpowers:verification-before-completion`')
