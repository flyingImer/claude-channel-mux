...
test('ask_peer is an async same-room peer handoff tool, not daemon memory', () => {
  ...
  expect(daemon).toContain('const nativeTurnId = await agentRegistry.get(peer).sendTurn({ session, turn })')
...
test('reply delivery preserves Slack thread broadcast and safe fallback parity', () => {
  ...
  expect(daemon).toContain('let replyTo = optionalString(msg.args.reply_to)')
