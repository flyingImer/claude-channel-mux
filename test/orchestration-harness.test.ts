...
test('portable prompt pack maps orchestration roles to bundled skills', () => {
  ...
  expect(readFileSync('prompts/ccm/orchestrator.md', 'utf8')).toContain('Claude worker goal setup must be sent with `send_worker_raw_command` as `/goal create dynamic workflow to <task specific goal description> /think-harder /superpowers:verification-before-completion`')
  expect(readFileSync('prompts/ccm/orchestrator.md', 'utf8')).toContain('Codex worker goal setup must be sent with `send_worker_raw_command` as `/goal $superpowers:subagent-driven-development <task specific goal description> $think-harder $superpowers:verification-before-completion`')
  expect(readFileSync('prompts/ccm/orchestrator.md', 'utf8')).toContain('send_worker_task carries only the task-specific Worker Task brief')
  ...
  expect(chatgptPrompt).toContain('Claude worker goal setup must be sent via ACP `send_worker_raw_command` with `/goal create dynamic workflow to <task specific goal description> /think-harder /superpowers:verification-before-completion`')
  expect(chatgptPrompt).toContain('Codex worker goal setup must be sent via ACP `send_worker_raw_command` with `/goal $superpowers:subagent-driven-development <task specific goal description> $think-harder $superpowers:verification-before-completion`')
  expect(chatgptPrompt).toContain('send_worker_task should carry only the task-specific Worker Task brief')
...
test('orchestration checklists cover preflight dispatch integration and recovery gates', () => {
  ...
  expect(readFileSync('docs/checklists/worker-dispatch.md', 'utf8')).toContain('Runtime-specific `/goal` setup is sent with `send_worker_raw_command` before the Worker Task brief')
