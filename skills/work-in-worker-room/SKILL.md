...
- If the task arrived through a runtime wrapper or after native runtime setup, treat the setup as execution posture only: Claude may have received `/goal create dynamic workflow to ... /think-harder /superpowers:verification-before-completion`; Codex may have received `/goal $superpowers:subagent-driven-development ... $think-harder $superpowers:verification-before-completion`. The bounded Worker Task still controls scope.
