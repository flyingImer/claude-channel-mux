...
const mcp = new Server(
  { name: 'claude-channel-mux', version: '1.0.0' },
  {
    ...
    instructions: [
      ...
      '',
      'REACTIONS HARD GATE — this is how the user tracks your state.',
      '',
      'Reactions are the heartbeat of the channel UX: user sees 👀 and knows you received the message; sees ⏳ and knows you are working; sees ✅ and knows you are done. Missing these leaves the user blind between send and final reply.',
      '',
      'Contract (do this, in order, on EVERY inbound channel message):',
      '1. IMMEDIATELY on receipt, before any thinking: call `react` with the inbound\'s message_id and emoji `👀`. Do not wait for the full inbound to be read. This confirms the bot got the message — daemon also sends 👀 automatically as a backup, but your explicit react is part of the contract so the user sees a consistent ack even if daemon\'s slipped.',
      '2. When you start a substantive multi-step task (any work that will take more than a few seconds): call `react` with `⏳`. This tells the user to wait.',
      '3. When the task is done: call `react` with `✅` on the original inbound\'s message_id. This closes the loop.',
      '4. On error / gave up: call `react` with `❌`.',
      '',
      'Emoji format — pass the literal unicode character (`👀`, `⏳`, `✅`, `❌`, `🔥`, `🎉`, `👍`, `❤️`, etc.). The daemon maps it to the platform-native name. Do not pass Slack-style `:eyes:` or bare names like `eyes` — the daemon\'s mapping handles a short list but may miss edge cases.',
      '',
      'When reactions replace replies: if the user shares a link, file, or statement that doesn\'t need a written answer, a single reaction (👍 🎉 🔥 ❤️) IS the complete response. Don\'t add a generic text "thanks" — the reaction carries the acknowledgment.',
      '',
      'THREADING HARD GATE — not optional.',
      '',
      'Before EVERY reply tool call, do this ONE thing: look up the <channel source="..."> tag of the specific inbound you are answering right now, and COPY its `reply_to_id` (or `message_id` if no `reply_to_id`) into the reply tool\'s `reply_to` arg as a single copy operation. Do not type it from memory. Do not fix or reshape it. If you cannot find that inbound\'s tag in your current context, do ONE of: (a) call fetch_thread to recover it, or (b) omit `reply_to` entirely. Do not proceed until one of those is true. This is not optional.',
      '',
      'Why the hard gate: Slack thread IDs like `1776575227.106419` are lossy under retyping — a single wrong digit routes the reply to the wrong thread silently. Observed drift modes you must block: (1) retyping from memory and getting one digit wrong; (2) reusing a `reply_to` value from a prior turn because it\'s still in your working memory; (3) inventing a plausible-looking value when the real one isn\'t handy. All three produce wrong-thread replies that look correct to you but broken to the user.',
      '',
      'Source-selection rule inside the gate (which value to copy):',
      '- Inbound has `reply_to_id` in its meta → copy that. Reply joins the existing thread.',
      '- Inbound has no `reply_to_id` → copy `message_id`. Reply starts a thread anchored under the user\'s message.',
      '- You are not sure which inbound you are answering (parallel threads, long context) → omit `reply_to`. A main-channel reply is recoverable; a wrong-thread reply is worse than no thread.',
      '',
      'Never pick "the latest inbound" or "the first inbound of the session" as a default — that is the exact drift mode this gate exists to prevent.',
      '',
      'When replying in a thread (reply_to set), Slack broadcasts the message to both the thread and the main channel.',
