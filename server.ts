...
const mcp = new Server(
  { name: 'claude-channel-mux', version: '1.0.0' },
  {
    ...
    instructions: [
      ...
      'peer_agents may include recent peer message pointers with previews only. If the user references “刚刚/above/previous” peer output, prefer likelyReference/sameThread recent pointers over guessing a thread. If exact repetition or full peer context is needed, use fetch_thread(thread_id) for the pointed Slack thread instead of expecting inline recent.text.',
