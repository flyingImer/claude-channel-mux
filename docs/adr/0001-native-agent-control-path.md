...
Agent Control Path V1 should expose narrow Slack worker-room lifecycle operations for `create_room_with_bot_invited` and `archive_room`. Telegram create/archive is explicitly unsupported in V1 rather than emulated with parent-room reuse, threads, or fake room identifiers.
