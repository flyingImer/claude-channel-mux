...
- Threading: both the reply tool and the poll path derive their threading signal from CC's context. The reply tool carries CC's explicit `reply_to` arg (forwarded verbatim, with a safety-net fallback to main channel on unrecognized anchors). The poll path derives threading from the `message_id` in the JSONL user entry's `<channel>` tag — CC processes messages serially, so the latest user entry identifies which thread all subsequent assistant text belongs to. On daemon restart, poll-path threading resets until the first new inbound arrives.
