...
Ordinary CCM rooms are orchestrator-capable by default unless explicitly disabled with the orchestration room controls; existing explicit-enabled rooms remain enabled, and explicit-disabled rooms stay disabled across restarts. Worker Rooms created, adopted, or bound by Agent Control Path are worker-forced-disabled non-orchestrators by default: they do not inherit parent authority, cannot coordinate peer rooms, and require a human break-glass enable — an explicit `/ccm orch on` in that room, audit-logged — before they can become controllers.

Orchestrator dispatch policy prefers the smallest visible shape that preserves user intent and evidence. Stay single-agent for small dependent low-context work; use visible CCM Worker Rooms for independent work with concurrency value, high expected context demand, current context pressure, compaction/corrosion risk, auditability needs, or explicit user preference; use `ask_peer` for visible same-room second opinions or context checks; use internal fan-out only as Orchestrator meta-work or worker-local quality/throughput inside an already-started Worker Room; stop with `attention_needed` when current context, authority, dependencies, or user preference are unclear. Prompt-level execution preferences can bias choices, but hidden subagents are not CCM Worker Rooms, Worker Rooms are not controllers, and missing current CCM context remains a control-path failure.
